...
% output is a vector containing the length of an island followed by its
% indices.
...
chain = [];
index_total = length(index_copy); 

% Loop over each individual pixel index
for q1 = 1:index_total 
    ...
    if isfinite(index_copy(q1)) == 1 
        ...
        while isempty(kk) == 0 
                ...
                % index, and eventually checking the indicies around the
